...
def get_children_probability(state: int, turn: str, chosen_column: int):
    ...
    if chosen_column < COLUMNS - 1 and columns_count[chosen_column] < ROWS:
        ...
        children.append((int(new_state), 0.2))
...
def heuristic(board):
    ...
    w3 = 4
    w2 = 2
    agent_4s = count_diagonal_disks(board, 1, 4) + count_vertical_disks(board, 1, 4) + count_horizontal_disks(board, 1, 4)
    agent_3s = count_diagonal_disks(board, 1, 3) + count_vertical_disks(board, 1, 3) + count_horizontal_disks(board, 1, 3)
    agent_2s = count_diagonal_disks(board, 1, 2) + count_vertical_disks(board, 1, 2) + count_horizontal_disks(board, 1, 2)
    player_4s = count_diagonal_disks(board, 0, 4) + count_vertical_disks(board, 0, 4) + count_horizontal_disks(board, 0, 4)
    player_3s = count_diagonal_disks(board, 0, 3) + count_vertical_disks(board, 0, 3) + count_horizontal_disks(board, 0, 3)
    player_2s = count_diagonal_disks(board, 0, 2) + count_vertical_disks(board, 0, 2) + count_horizontal_disks(board, 0, 2)
...
s = "000000000000000000000100020011202102212120"
board = [list(s[i * COLUMNS:(i + 1) * COLUMNS]) for i in range(ROWS)]
agent_4s = count_diagonal_disks(board, 1, 2) + count_vertical_disks(board, 1, 2) + count_horizontal_disks(board, 1, 2)
for row in board:
    print(row)
print(agent_4s)
# children = get_children(int(s), 1)
...
#     for i in range(ROWS):
#         print(state_str[i*COLUMNS:(i + 1) * COLUMNS])
#     print()